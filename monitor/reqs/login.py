...
class LoginReq:
    ...
    async def logout(user):
        ...
        json_rsp = await user.login_session.request_json('GET', url, headers=user.dict_bili['pcheaders'],
                                                         ctrl=LOGIN_CTRL)
        return json_rsp
    ...
    async def login(user, url_name, url_password, captcha=''):
        ...
        json_rsp = await user.login_session.request_json('POST', url, headers=user.dict_bili['appheaders'],
                                                         params=params, ctrl=LOGIN_CTRL)
        return json_rsp
    ...
    async def is_token_usable(user):
        ...
        json_rsp = await user.login_session.request_json('GET', true_url, params=params,
                                                         headers=user.dict_bili['appheaders'], ctrl=LOGIN_CTRL)
        return json_rsp
